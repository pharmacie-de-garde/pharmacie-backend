...
export class FavoritePharmacyService
  implements FavoritePharmacyServiceInterface
{
  constructor(
    @Inject('FavoritePharmacyRepositoryInterface')
    private readonly favoritePharmacyRepository: FavoritePharmacyRepositoryInterface,
    private readonly farmacyService: PharmaciesService,
  ) {}

  async handelCreateFavoritePharmacy(
    createFavoritePharmacyDTO: CreateFavoritePharmacyDTO,
    userId: Types.ObjectId,
  ): Promise<FavoritePharmacyEntity> {
    const findPharmacy = await this.farmacyService.findById(
      createFavoritePharmacyDTO.pharmacy_id,
    );
    if (!findPharmacy) {
      throw new HttpException('Pharmacy not found', HttpStatus.NOT_FOUND);
    }
    const favoriteAlredyExists =
      await this.favoritePharmacyRepository.findFavoritePharmacyByIdAndUserId(
        createFavoritePharmacyDTO.pharmacy_id,
        userId,
      );
    if (favoriteAlredyExists) {
      throw new HttpException(
        'Favorite Pharmacy already exists',
        HttpStatus.CONFLICT,
      );
    }
    const favoritePharmacy =
      await this.favoritePharmacyRepository.createFavoritePharmacy(
        createFavoritePharmacyDTO,
        userId,
      );
    if (!favoritePharmacy) {
      throw new HttpException(
        'Favorite Pharmacy not created',
