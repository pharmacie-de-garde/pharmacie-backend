...
@Injectable()
export class FavoritePharmacyRepository
  implements FavoritePharmacyRepositoryInterface
{
  constructor(
    @InjectModel(FavoritePharmacy.name)
    private readonly favoritePharmacyModel: Model<FavoritePharmacy>,
  ) {}

  async findFavoritePharmacyByIdAndUserId(
    pharmacyId: Types.ObjectId,
    userId: Types.ObjectId,
  ): Promise<Partial<FavoritePharmacyEntity>> {
    return await this.favoritePharmacyModel
      .findOne({
