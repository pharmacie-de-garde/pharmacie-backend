...
export class FavoritePharmacyController {
  constructor(
    @Inject('FavoritePharmacyServiceInterface')
    private readonly favoritePharmacyService: FavoritePharmacyServiceInterface,
  ) {}
  ...
  async createFavoritePharmacy(
    ...
    try {
      ...
      const favoritePharmacy =
        await this.favoritePharmacyService.handelCreateFavoritePharmacy(
          createFavoritePharnacyDTO,
          userId,
        );
      return {
